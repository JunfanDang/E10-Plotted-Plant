...
define( 'HIDE_METADATA_VERSION', '1.1' );
...
class Hide_Metadata {
	...
	public function sanitize_callback( $input ) {
		...
		if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )
			|| ( ! isset( $_POST['hide_metadata_nounce'] )
			|| ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['hide_metadata_nounce'] ) ), basename( __FILE__ ) ) )
			|| ( ! check_admin_referer( basename( __FILE__ ), 'hide_metadata_nounce' ) ) ) {
			if ( null !== $input ) {

				$input['hide_by']     = sanitize_key( $input['hide_by'] );
				$input['hide_author'] = ( isset( $input['hide_author'] ) && 'on' == $input['hide_author'] ) ? '1' : '0';
				$input['hide_date']   = ( isset( $input['hide_date'] ) && 'on' == $input['hide_date'] ) ? '1' : '0';
	...
	function inline_style() {
		if ( ! empty( $this->hide_by_css() ) ) {
			echo '<style>/* CSS added by Hide Metadata Plugin */' . $this->hide_by_css() . '</style>';
		}
	}
